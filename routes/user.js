...
userRouter.post("/api/order", auth, async (req, res) => {
  try {
    const { cart, totalPrice, payment, address,isDirect } = req.body;
    let products = [];
...
userRouter.post("/api/add-to-wishlist", auth, async (req, res) => {
  try {
    ...
    let isFound = false;
    for (let index = 0; index < user.favorite.length; index++) {
      if (user.favorite[index].product._id.equals(product._id)) {
        isFound = true;
      }
    }
